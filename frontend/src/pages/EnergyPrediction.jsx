...
function EnergyPrediction({ liveData }) {
  ...
  const [viewMode, setViewMode] = useState('model'); // 'model' or 'database'
  const [savedPredictions, setSavedPredictions] = useState([]);
  const [savedPredictionsLoading, setSavedPredictionsLoading] = useState(false);
  ...
  const fetchSavedPredictions = async () => {
    setSavedPredictionsLoading(true);
    try {
      const data = await api.getSavedPredictions(horizon);
      setSavedPredictions(data);
    } catch (err) {
      console.error("Failed to fetch saved predictions:", err);
    } finally {
      setSavedPredictionsLoading(false);
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold flex items-center gap-2">
                ...
                <span>Database Stored AI Predictions</span>
              </h3>
              <p className="text-slate-400 text-xs mt-0.5">
                Query history of stored predictive logs populated by live inference or CSV upload.
              </p>
            </div>
            <button
              onClick={fetchSavedPredictions}
              className="p-2 border border-slate-800 bg-slate-950 hover:bg-slate-900 text-slate-400 hover:text-white rounded-xl transition-all"
            >
              <RefreshCw size={14} className={savedPredictionsLoading ? 'animate-spin' : ''} />
            </button>
          </div>

          <div className="flex items-center gap-3">
            <div className="flex-1">
              <span className="text-[10px] font-bold text-slate-500 uppercase">Horizon Filter</span>
              <select
                value={horizon}
                onChange={(e) => setHorizon(e.target.value)}
                className="w-full mt-1 px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none"
              >
                <option value="hour">Hourly Logs (Hour)</option>
                <option value="day">Daily Logs (Day)</option>
                <option value="week">Weekly Logs (Week)</option>
                <option value="month">Monthly Forecast (Month)</option>
              </select>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs font-semibold">
              ...
              <tbody>
                {savedPredictionsLoading ? (
                  ...
                  <tr>
                    <td colSpan="5" className="p-8 text-center text-slate-500">
                      No prediction logs found in database. Ingest prediction CSVs or query models to generate them.
                    </td>
                  </tr>
                ) : (
                  savedPredictions.map((row) => (
                    <tr key={row.id} className="border-b border-slate-900 hover:bg-slate-900/20 text-slate-300">
                      <td className="p-3 font-mono text-slate-500">{row.id}</td>
                      <td className="p-3 text-slate-200">{row.timestamp}</td>
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                          row.target === 'total_power' ? 'bg-cyan-500/10 text-cyan-400' : 'bg-yellow-500/10 text-yellow-400'
                        }`}>
                          {row.target}
                        </span>
                      </td>
                      <td className="p-3 font-bold text-slate-100">{row.value} kW</td>
                      <td className="p-3 uppercase text-[10px] font-extrabold text-slate-400">{row.horizon}</td>
                    </tr>
                  ))
                )}
              </tbody>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-slate-800 bg-slate-900/30 shadow-md space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
                ...
                <div className="p-5 bg-slate-950/45 border border-slate-850 rounded-2xl flex flex-col justify-between">
                  <div>
                    <h4 className="text-xs font-bold text-slate-355 uppercase tracking-wider mb-4">
                ...
                <div className="p-5 bg-slate-950/45 border border-slate-850 rounded-2xl flex flex-col justify-between">
                  <div>
                    <h4 className="text-xs font-bold text-slate-355 uppercase tracking-wider mb-4 flex items-center gap-1.5">
